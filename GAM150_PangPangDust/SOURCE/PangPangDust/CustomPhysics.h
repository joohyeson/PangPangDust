...
public:
	...
	bool IsCollidingExcept(CustomBaseObject* other) const;
	...
	b2Vec2 GetBoxHalfScale() const;
	// Set position of body
	void SetPosition(float x, float y);
	// Set scale of body
	void SetBoxScale(float x, float y);

	// Create fixture for sensor and attach it to present body
	void GenerateSensor(float positionX, float positionY, eContactTypes sensorType);
	// Set contact data to main fixture
	void SetContactUserData(ContactData* userData);
	ContactData*                mContactData = nullptr;
	// Sensor fixture's contact data
	std::vector<ContactData*>   mSensorContactData;
