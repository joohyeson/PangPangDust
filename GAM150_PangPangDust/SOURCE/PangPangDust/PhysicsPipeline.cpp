...
void State::SetCustomContactListener(b2ContactListener* listener)
{
    m_customListener = listener;
    m_customWorld->SetContactListener(m_customListener);
}
...
/**
* \brief
* Deallocate custom physics component from every object
*
* \param physics
* Address of custom physics component
*/
void State::RemoveCustomPhysicsComponent(CustomPhysics* physics)
{
    if (!m_useBuiltInPhysics) {

        for (auto element = m_customPhysicsList.begin();
            element != m_customPhysicsList.end(); ++element) {
            if (physics == *element) {
                m_customPhysicsList.erase(element);
                break;
            }
        }
